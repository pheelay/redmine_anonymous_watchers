...
Anonymous Watchers plug-in allows to add any e-mail addresses as watchers in Redmine.
When creating issues from mails, it also adds non-member e-mails to watchers automatically (exceptions can be configured).
...
This plug-in rewrites the following views:
...
* Restart Redmine.
* Configure e-mail exceptions list, if you use issues creation from mails
